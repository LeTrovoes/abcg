...
void Car::computeNormals() {
  ...
  for (const auto offset : iter::range<int>(0, indexes.size(), 3)) {
    // Get face vertices
    Vertex& a{m_vertices.at(indexes.at(offset + 0))};
    Vertex& b{m_vertices.at(indexes.at(offset + 1))};
    Vertex& c{m_vertices.at(indexes.at(offset + 2))};
...
void Car::createBuffers() {
  ...
  abcg::glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                     sizeof(indexes[0]) * indexes.size(), indexes.data(),
                     GL_STATIC_DRAW);
  abcg::glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
}

void Car::loadDiffuseTexture(std::string_view path) {
...
void Car::render(int numTriangles) const {
  ...
  const auto numIndices{(numTriangles < 0) ? indexes.size()
                                           : numTriangles * 3};
